...
│   │   ├── auth/
│   │   │   ├── interfaces.ts         ← AuthProvider interface
│   │   │   ├── firebase-client.ts
...
| `/home` 共振 Feed | **Server Component** | 第一階段顯示最新公開卡片；第二階段改為每日推薦 feed。推薦 feed 非 ISR — 每人不同 |
...
- [ ] Auth Provider abstraction + Firebase Auth：Email + Phone OTP 註冊 / 登入 (M9)
- [ ] Firestore schema + security rules (含 aggregate 私密化)
- [ ] 各 Repository 的 Firestore 實作，支援 provider 切換
- [ ] Storage Provider abstraction + `/api/upload` → R2 presigned URL
- [ ] CardEditor 支援圖片上傳與媒體 metadata
- [ ] `/home` 使用 latest published feed：公開卡片依 `publishedAt desc` 顯示
- [ ] Notification pipeline：invite / resonance summary / invite expired
- [ ] 配額 middleware (`lib/quota/`)：連結邀請每日 3 次
